...
void cmd_help_h(const struct cmd_tokenized_node* tokens) {
	...
	io_printfln("/nice <action> <jid>");
	io_printfln("\tbefore call a /nice session must select the jid you want to communicate with");
	io_printfln("\taction could be --> \"request\" ");
	io_printfln("\taction could be --> \"accept\"");
	io_printfln("\taction could be --> \"denied\"");
	io_printfln("\taction could be --> \"end\"");
